...
if __name__ == "__main__":
    ...
    model.regressor.smpl = SMPL(SMPL_MODEL_DIR, batch_size=64, create_transl=False, gender=gender).cuda()
    dtype = torch.float
    ...
    if target_dataset == "3dpw":
        ...
        if cfg.TITLE == "repr_table4_h36m_mpii3d_model":
            data_path = (
                f"C:\\Users\\marcw\\master_thesis\\forked\\GLoT\\data\\preprocessed_data\\{target_dataset}_{set}_25fps_db.pt"  # Table 4
            )
    ...
    with torch.no_grad():
        ...
        for seq_name in pbar:
            ...
            chunk_idxes = split_into_chunks_test(
            ...
            if render:
                ...
                for ii in tqdm(range(len(imgname))):
                    ...
                    orig_cam = convert_crop_cam_to_orig_img(
                        cam=cam, bbox=bbox_ii, img_width=orig_width, img_height=orig_height
                    )
                    ...
                    new_h, new_w = new_h if new_h % 2 == 0 else new_h - 1, (
                        new_w if new_w % 2 == 0 else new_w - 1
                    )  # for ffmpeg
